...
const createPayment = async(userId: string, isTenant: boolean, rentalRequestId: string)=>{
    ...
    const amount = Math.round(Number(paymentData.rentalReq.property.rent)) * 100;
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            success_url: `${config.app_url}/payment/success?rentalRequestId=${paymentData.rentalReq.id}`,
            cancel_url: `${config.app_url}/payment/cancel?rentalRequestId=${paymentData.rentalReq.id}`,
            metadata: {
